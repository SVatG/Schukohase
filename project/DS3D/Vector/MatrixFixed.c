...
imat4x3_t imat4x3rotate(int angle,ivec3_t axis) { return imat4x3affine3x3(imat3x3rotate(angle,axis)); }

imat4x4_t imat4x4rotate(int angle,ivec3_t axis) { return imat4x4affine3x3(imat3x3rotate(angle,axis)); }
...
{
	imat2x2_t res;
	int32_t det=imul(m.m[0],m.m[2])-imul(m.m[1],m.m[2]);
	// singular if det==0

	res.m[0]=idiv(m.m[0],det);
	res.m[2]=idiv(m.m[1],det);

	res.m[1]=idiv(m.m[2],det);
...
{
	imat3x3_t res;
	int32_t det=imul(imul(m.m[0],m.m[4]),m.m[8])-imul(imul(m.m[0],m.m[5]),m.m[7])+
	            imul(imul(m.m[1],m.m[5]),m.m[6])-imul(imul(m.m[1],m.m[3]),m.m[8])+
			    imul(imul(m.m[2],m.m[3]),m.m[7])-imul(imul(m.m[2],m.m[4]),m.m[6]);
	// singular if det==0
...
{
	imat3x3_t res;
	int32_t det=imul(m.m[0],m.m[3])-imul(m.m[1],m.m[4]);
	// singular if det==0

	res.m[0]=idiv(m.m[0],det);
	res.m[3]=idiv(m.m[1],det);

	res.m[1]=idiv(m.m[3],det);
	res.m[4]=idiv(m.m[4],det);
...
{
	return ivec2(
	imul(v.x,m.m[0])+imul(v.y,m.m[2]),
	imul(v.x,m.m[1])+imul(v.y,m.m[3]));
}

ivec2_t imat3x2transform(imat3x2_t m,ivec2_t v)
{
	return ivec2(
	imul(v.x,m.m[0])+imul(v.y,m.m[2])+m.m[4],
	imul(v.x,m.m[1])+imul(v.y,m.m[3])+m.m[5]);
}
