...
dmat4x3_t dmat4x3rotate(double angle,dvec3_t axis) { return dmat4x3affine3x3(dmat3x3rotate(angle,axis)); }

dmat4x4_t dmat4x4rotate(double angle,dvec3_t axis) { return dmat4x4affine3x3(dmat3x3rotate(angle,axis)); }
...
{
	dmat2x2_t res;
	double det=m.m[0]*m.m[2]-m.m[1]*m.m[2];
	// singular if det==0
...
{
	dmat3x3_t res;
	double det=m.m[0]*m.m[4]*m.m[8]-m.m[0]*m.m[5]*m.m[7]+
...
{
	dmat3x3_t res;
	double det=m.m[0]*m.m[3]-m.m[1]*m.m[4];
	// singular if det==0
...
{
	dmat4x4_t res;
	double det=m.m[0]*m.m[5]*m.m[10]-m.m[0]*m.m[6]*m.m[9]+
